...
def build_intent_chat_system_prompt(dataset_name: str, columns: list[dict]) -> str:
    ...
    return (
        ...
        "entity_id_column, retrain_cadence, score_cadence). Rules:\n"
        "- candidate_features must only ever be drawn from the real column "
        "names listed above -- never invent one.\n"
        "- entity_id_column: if the dataset is a panel/repeated-observations "
        "table -- the same real-world entity appears in more than one row, "
        "e.g. one row per county per year -- set this to the column that "
        "names the entity (e.g. a FIPS code or customer ID), even though its "
        "values repeat across rows. Leave it null for a plain "
        "one-row-per-entity table.\n"
